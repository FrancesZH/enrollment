        ...
        while (getline(schedule, line))
        {
            if (line != "")
            {
                ...
                else
                {
                    ...
                    {
                        bool allPrereqsTakenInVector = true;
                        for (const string &prereq : prereqVector)
                        {
                            if (!courses.count(prereq))
                            {
                                allPrereqsTakenInVector = false;
                                cout << course << " Prerequisite not taken: " << prereq << std::endl;
                                break; // Exit the inner loop as soon as a prerequisite that has not been taken is found
                            }
                        }
                        if (!allPrereqsTakenInVector)
                        {
                            allPrereqsTaken = false;
                            break; // Exit the outer loop as soon as a vector of prerequisites that has not been taken is found
                        }
                    }
                    if (allPrereqsTaken)
